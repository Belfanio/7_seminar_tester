...
def word(words):
    input_string = words.lower().split()
    f = lambda x: sum(1 for i in x if i in 'аоуыэеёиюя')
    tmp = f(input_string[0])
    if all([f(i) == tmp for i in input_string]):
        return 'Парам пам-пам'
    return 'Пам парам'


input_string = input('Введите строку для проверки рифмы: ')
print(word(input_string))
